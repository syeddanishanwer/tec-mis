    ...
    <div className="space-y-4">
      <div className="bg-white p-4 rounded-xl border shadow-xs space-y-3">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 pt-2 border-t">
          <div><label className="text-xs font-semibold block mb-1"><Filter className="w-3 h-3 inline mr-1" />Class</label>
            <select value={selectedClass} onChange={e => setSelectedClass(e.target.value)} className="w-full text-xs bg-neutral-50 border rounded-lg px-2.5 py-1.5"><option value="ALL">All Classes</option>{ALL_CLASSES.map(c => <option key={c} value={c}>{c}</option>)}</select></div>
          <div><label className="text-xs font-semibold block mb-1">Active Month</label>
            <select value={activeReminderMonth} onChange={e => handleActiveMonthChange(e.target.value as AcademicMonth)} className="w-full text-xs bg-neutral-50 border rounded-lg px-2.5 py-1.5">{ACADEMIC_MONTHS.map(m => <option key={m} value={m}>{m}</option>)}</select></div>
          <div><label className="text-xs font-semibold block mb-1">Status ({activeReminderMonth})</label>
            <select value={selectedStatus} onChange={e => setSelectedStatus(e.target.value)} className="w-full text-xs bg-neutral-50 border rounded-lg px-2.5 py-1.5"><option value="ALL">All</option><option value="unpaid">Unpaid</option><option value="partial">Partial</option><option value="paid">Paid</option><option value="new_admission">New Admission</option></select></div>
          <div><label className="text-xs font-semibold block mb-1">Search</label>
            <div className="relative"><Search className="w-3.5 h-3.5 absolute left-2.5 top-2.5 text-neutral-400" /><input type="text" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder="roll, name, father..." className="w-full pl-8 pr-3 py-1.5 text-xs bg-neutral-50 border rounded-lg" /></div></div>
        </div>
      </div>
          ...
          <table className="w-full text-left border-collapse text-xs" style={{ minWidth: `${780 + visibleMonths.length * 68}px` }}>
            <thead>
              <tr className="bg-neutral-900 text-white font-semibold text- uppercase tracking-wider">
                <th onClick={() => handleSort('id')} className="py-3 px-3 cursor-pointer w-12 text-center"><div className="flex items-center justify-center gap-1">S#<ArrowUpDown className="w-3 h-3" /></div></th>
                ...
                {visibleMonths.map(m => <th key={m} className={`py-3 px-1.5 text-center min-w- ${m === activeReminderMonth? 'bg-blue-900/80' : ''}`}>{m}</th>)}
            ...
            <tbody className="divide-y divide-neutral-200/70">
              {filteredStudents.length === 0? <tr><td colSpan={9 + visibleMonths.length} className="py-12 text-center text-neutral-400">No records</td></tr> : (
                <>
                  {classGroups.map((group: any) => (
                    <React.Fragment key={group.className}>
                      ...
                      <tr key={`total-${group.className}`} className="bg-neutral-100/90 font-semibold border-t-2 border-b">
                        <td colSpan={5} className="py-2.5 px-3 text-xs"><div className="flex items-center justify-between"><span className="font-bold uppercase">Total — {group.className}</span><span className="text- bg-white px-2 py-0.5 rounded border">{group.students.length} students</span></div></td>
                        <td className="py-2.5 px-3 text-right font-mono text-xs">Rs. {group.totals.monthlyFee.toLocaleString()}</td>
                        {visibleMonths.map(m => { const mData = group.totals.months[m] || { collected: 0, due: 0 }; return <td key={`ct-${group.className}-${m}`} className="py-2 px-1 text-center font-mono text-xs"><span className="font-bold text-emerald-700 block text-">{mData.collected.toLocaleString()}</span><span className="text- text-rose-600 block">Due: {mData.due.toLocaleString()}</span></td>; })}
                  ...
                  <tr className="bg-neutral-900 text-white font-bold border-t-2">
                    ...
                    {visibleMonths.map(m => { const mData = overallTotals.months[m] || { collected: 0, due: 0 }; return <td key={`ot-${m}`} className="py-3 px-1 text-center font-mono text-xs"><span className="font-bold text-emerald-400 block text-">{mData.collected.toLocaleString()}</span><span className="text- text-rose-300 block">Due: {mData.due.toLocaleString()}</span></td>; })}
