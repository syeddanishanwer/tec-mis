...
import { parseExcelOrCsvFile, downloadSampleImportTemplate, ParsedImportRow, convertParsedRowToStudentRecord } from '../utils/importHelpers';
import { ACADEMIC_MONTHS } from '../types';
import { Upload, FileSpreadsheet, Download, CheckCircle2, AlertCircle, AlertTriangle, X, RefreshCw, Layers, ArrowRight } from 'lucide-react';

// FIXED: Local helper - no mockStudents import
...
export const ImportStudentsModal: React.FC<Props> = ({
  ...
  const handleProcessFile = async (selectedFile: File) => {
    ...
    try {
      const result = await parseExcelOrCsvFile(selectedFile, targetYear, existingStudents.length + 1);
      setParsedValidRows(result.validRows);
      setParsedInvalidRows(result.invalidRows);
      setGlobalWarnings(result.warnings);
      if (result.validRows.length === 0 && result.invalidRows.length === 0) setParseError('The file contains no readable student rows.');
    } catch (err: any) {
  ...
  // FIXED: Commit using invoices + feeSchedules, not old jsonb
  const handleCommitImport = () => {
    ...
    const newStudentRecords: any[] = parsedValidRows.map((row) => {
      ...
      return {
        ...
        feeChanges: row.feeChanges, // Keep for API compatibility
      };
    });
    ...
    onImportSuccess(
      ...
      strategy,
      { total: parsedValidRows.length, updated: updatedCount, added: addedCount },
      targetYear
    );
    onClose();
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-3 sm:p-4 backdrop-blur-xs">
      <div className="bg-white rounded-xl shadow-2xl border border-neutral-200 w-full max-w-5xl max-h- flex flex-col overflow-hidden animate-in fade-in duration-200">
        <div className="bg-neutral-900 px-6 py-4 text-white flex items-center justify-between shrink-0">
          <div className="flex items-center gap-3">
            <div className="p-2 bg-neutral-800 rounded-lg"><FileSpreadsheet className="w-5 h-5 text-emerald-400" /></div>
            <div>
              <h3 className="font-bold text-base leading-tight">Import Student Fee Ledger</h3>
              <p className="text-xs text-neutral-400">Excel (.xlsx,.xls) or CSV with NEW ADMISSION support</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <button onClick={downloadSampleImportTemplate} className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-neutral-800 hover:bg-neutral-700 text-neutral-200 border border-neutral-700 rounded-lg text-xs font-medium"><Download className="w-3.5 h-3.5 text-emerald-400" />Download Template</button>
            <button onClick={onClose} className="p-1.5 rounded-md text-neutral-400 hover:text-white hover:bg-neutral-800"><X className="w-5 h-5" /></button>
          </div>
        </div>
        ...
        <div className="p-5 overflow-y-auto flex-1 space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 bg-neutral-50 p-3.5 rounded-xl border border-neutral-200 text-xs">
            <div><label className="block font-semibold text-neutral-700 mb-1">Target Academic Session</label><select value={targetYear} onChange={e => setTargetYear(e.target.value)} className="w-full text-xs font-semibold bg-white border border-neutral-300 rounded-lg p-2 focus:ring-2 focus:ring-neutral-900 focus:outline-none">{yearsList.map(yr => <option key={yr} value={yr}>Session {yr}</option>)}</select></div>
            <div><label className="block font-semibold text-neutral-700 mb-1">Import Strategy</label><select value={strategy} onChange={e => setStrategy(e.target.value as any)} className="w-full text-xs font-semibold bg-white border border-neutral-300 rounded-lg p-2 focus:ring-2 focus:ring-neutral-900 focus:outline-none"><option value="update_or_add">Update existing & add new (Recommended)</option><option value="append">Append all as new students</option><option value="replace">Replace entire register</option></select></div>
          </div>

          {parseError && <div className="p-3 bg-red-50 border border-red-200 rounded-lg text-xs text-red-700 flex items-start gap-2"><AlertCircle className="w-4 h-4 shrink-0 mt-0.5 text-red-600" /><span>{parseError}</span></div>}
          {globalWarnings.length > 0 && <div className="p-2.5 bg-amber-50 border border-amber-200 rounded-lg text-xs text-amber-800 flex items-start gap-2"><AlertTriangle className="w-4 h-4 shrink-0 mt-0.5 text-amber-600" /><div>{globalWarnings.map((w, i) => <p key={i}>{w}</p>)}</div></div>}
          ...
          {parsedValidRows.length > 0 && (
            <div className="flex items-center justify-between bg-emerald-50/70 border border-emerald-200 px-4 py-2.5 rounded-lg text-xs">
              <div className="flex items-center gap-2"><CheckCircle2 className="w-4 h-4 text-emerald-600" /><span className="font-semibold text-emerald-900">{parsedValidRows.length} Students ready to import</span>{parsedInvalidRows.length > 0 && <span className="text-red-600 font-medium">({parsedInvalidRows.length} rows skipped)</span>}</div>
              <span className="text-neutral-500">Total fees: Rs. {parsedValidRows.reduce((acc, r) => acc + r.monthlyFee, 0).toLocaleString()}</span>
            </div>
          )}

          {parsedValidRows.length > 0 && (
            <div className="border border-neutral-200 rounded-lg overflow-hidden">
              <div className="bg-neutral-100 px-3 py-2 border-b border-neutral-200 flex items-center justify-between text-xs font-semibold text-neutral-700"><span>Data Preview - NEW ADMISSION handled</span><span className="text- text-neutral-500 font-normal">Showing {parsedValidRows.length} rows</span></div>
              <div className="overflow-x-auto max-h-64">
                <table className="w-full text-left text-xs border-collapse">
                  <thead className="bg-neutral-900 text-white text- uppercase tracking-wider sticky top-0 z-10">
                    <tr><th className="py-2 px-2 text-center w-10">S#</th><th className="py-2 px-2">Class</th><th className="py-2 px-2">Roll No</th><th className="py-2 px-2">Student</th><th className="py-2 px-2">Father</th><th className="py-2 px-2">Contact 1</th><th className="py-2 px-2">Contact 2</th><th className="py-2 px-2 text-right">M. Fee</th>{ACADEMIC_MONTHS.map(m => <th key={m} className="py-2 px-1 text-center min-w-">{m}</th>)}</tr>
                  </thead>
                  <tbody className="divide-y divide-neutral-200">
                    {parsedValidRows.map(row => (
                      <tr key={row.rowNumber} className="hover:bg-neutral-50">
                        <td className="py-2 px-2 text-center font-mono font-bold text-neutral-700 bg-neutral-50">{row.serialNo}</td>
                        <td className="py-2 px-2 font-medium text-neutral-800 whitespace-nowrap">{row.className}</td>
                        <td className="py-2 px-2 font-mono text- text-neutral-600 whitespace-nowrap">{row.rollNo}</td>
                        ...
                        {ACADEMIC_MONTHS.map(m => {
                          ...
                          if (status === 'new_admission') {
                            return <td key={m} className="py-1.5 px-1 text-center whitespace-nowrap"><div className="px-1.5 py-0.5 rounded text- font-bold border bg-black text-white">NEW ADMISSION</div></td>;
                          }
