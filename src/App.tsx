...
export default function App() {
  ...
  const handleToggleMonthStatus = async (studentId: number, month: AcademicMonth) => {
    ...
    if (currentInvoice.isWaived) { showToast(`Cannot change ${month}: WAIVED. Remove waiver first.`); return; }
  ...
  // NEW: Toggle a month's Waived state on/off, independent of paid/partial/unpaid/new_admission status.
  const handleToggleWaived = async (studentId: number, month: AcademicMonth) => {
    const student = students.find(s => s.id === studentId);
    const currentInvoice = student?.invoices?.find(inv => inv.month === month && inv.academicYear === selectedAcademicYear);
    if (!currentInvoice) { showToast(`No invoice for ${month}`); return; }

    const newIsWaived = !currentInvoice.isWaived;
    const originalStudents = students;

    // Optimistic update: mirrors what the server will compute, so the ledger updates instantly.
    setStudents(prev => prev.map(s => {
      if (s.id !== studentId) return s;
      return {
        ...s,
        invoices: s.invoices.map(inv => {
          if (inv.month !== month || inv.academicYear !== selectedAcademicYear) return inv;
          if (newIsWaived) {
            return { ...inv, isWaived: true, netDue: 0 };
          }
          const isNewAdmissionMonth = inv.status === 'new_admission';
          const restoredNetDue = isNewAdmissionMonth
            ? 0
            : Math.max(0, Number(inv.baseFee) - Number(inv.concessionAmount || 0));
          return { ...inv, isWaived: false, netDue: restoredNetDue };
        }),
      };
    }));
    ...
    try {
      ...
      if (!res.ok) {
        ...
        showToast(newIsWaived ? `${month} marked as Waived` : `${month} waiver removed`);
      }
    } catch {
      setStudents(originalStudents);
    ...
    <div className="min-h-screen bg-neutral-100 text-neutral-900 font-sans flex flex-col">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16 gap-3">
            <div className="flex items-center gap-3 min-w-0">
              <div className="w-11 h-11 rounded-lg bg-white p-0.5 flex items-center justify-center overflow-hidden border border-neutral-700 shrink-0">
              ...
              <div className="min-w-0">
                ...
                <p className="text-xs text-neutral-400 hidden sm:block">
                  <span className="text-indigo-300 font-medium">Enter to learn. Go forth to serve.</span>
                </p>
              </div>
            </div>

            <div className="flex items-center gap-2.5 shrink-0">
              <div className="hidden md:flex items-center bg-neutral-800/90 border border-neutral-700/80 rounded-lg p-1 text-xs">
                <div className="flex items-center gap-1.5 px-2 py-0.5 text-neutral-300 font-medium">
                  <Calendar className="w-3.5 h-3.5 text-indigo-400" /><span className="hidden lg:inline text-neutral-400">Session:</span>
      ...
      <footer className="border-t border-neutral-200 bg-white py-4 mt-8">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col sm:flex-row items-center justify-between text-xs text-neutral-500 gap-2">
          ...
          <p className="font-mono text-xs">Session: {selectedAcademicYear} • Live DB • wa.me Integrated • {students.length} Students</p>
        </div>
      </footer>
    </div>
  );
