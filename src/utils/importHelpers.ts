...
export function normalizeContactNumber(rawPhone: any, allowEmpty = false): string | undefined {
  ...
  if (!cleaned) return allowEmpty ? undefined : '923001234567';
  return cleaned;
}
...
function parseFeeChangeSlot(
  ...
  const matchedMonth = monthAliasMap[monthLower];
  if (!matchedMonth) {
    errors.push(`Fee Change Slot ${slotNumber}: "${monthStr}" is not a valid month.`);
    return null;
  }
  return { newFee: roundedFee, effectiveFromMonth: matchedMonth };
}
...
function validateFeeChangeSequence(slots: (FeeChange | null)[], errors: string[]): FeeChange[] {
  ...
  for (let i = 0; i < slots.length; i++) {
    if (slots[i] === null) sawBlank = true;
    else if (sawBlank) errors.push(`Fee Change Slot ${i + 1} is filled but an earlier slot is blank.`);
  }
  const filled = slots.filter((s): s is FeeChange => s !== null);
  for (let i = 1; i < filled.length; i++) {
    const prevIdx = ACADEMIC_MONTHS.indexOf(filled[i - 1].effectiveFromMonth);
    const currIdx = ACADEMIC_MONTHS.indexOf(filled[i].effectiveFromMonth);
    if (currIdx === prevIdx) errors.push(`Duplicate month: ${filled[i].effectiveFromMonth}`);
    else if (currIdx < prevIdx) errors.push(`Fee changes must be chronological.`);
  }
  return filled;
}
...
export async function parseExcelOrCsvFile(
  ...
  for (let r = headerRowIndex + 1; r < rawRows.length; r++) {
    ...
    let rollNo = rollCol !== -1 ? String(row[rollCol] || '').trim() : '';
    if (!rollNo) {
      rollNo = `TEC-${1000 + startingSerialIndex + validRows.length}`;
      warnings.push(`Auto Generated Roll No: ${rollNo}`);
    }

    const studentName = studentNameCol !== -1 ? String(row[studentNameCol] || '').trim() : String(row[3] || '').trim();
    if (!studentName) errors.push('Student Name is required.');

    let fatherName = fatherNameCol !== -1 ? String(row[fatherNameCol] || '').trim() : '';
    if (!fatherName) {
      fatherName = 'Guardian';
      warnings.push("Father Name missing; defaulted to Guardian.");
    }

    const contactNo = (normalizeContactNumber(phoneCol !== -1 ? row[phoneCol] : '', false) as string) || '923001234567';
    const contactNo2 = normalizeContactNumber(phoneCol2 !== -1 ? row[phoneCol2] : '', true);
...
export function downloadSampleImportTemplate(): void {
  ...
  const sampleRows = [
    ['001', 'Class X', 'TEC-1001', 'Muhammad Hamza', 'Tariq Mehmood', '03001234567', '', 6000, 6500, 'Sep', 7000, 'Nov', '', '', 'NEW ADMISSION', 'NEW ADMISSION', 'NEW ADMISSION', 6500, 6500, 7000, 7000, 7000, 7000],
    ['002', 'Class IX', 'TEC-1002', 'Ayesha Fatima', 'Nadeem Akhtar', '03219876543', '03211234567', 6000, '', '', '', '', '', '', 6000, 6000, 6000, 6000, 0, 0, 0, 0, 0, 0, 0, 0],
  ];
  const ws = XLSX.utils.aoa_to_sheet([headers, ...sampleRows]);
  const wb = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(wb, ws, 'Student Fee Ledger');
